...
const findClosestToMiddleLetterPosition = (
  matches: RegExpMatchArray[],
  middlePosition: number,
) => {
  const indexesDistance = matches.map(({ index = 0 }) =>
    Math.abs(index - middlePosition),
  );
...
const getLetterPosition = ({
  ...
  return (
    findClosestToMiddleLetterPosition(matches, middlePosition) || middlePosition
  );
};

const calculateMaxShifts = (
  questionsWithLetterPosition: QuestionWithLetterPosition[],
) =>
  questionsWithLetterPosition.reduce(
    (acc, { word, letterPosition }) => {
...
const calculateShifts = (
  ...
  questionsWithLetterPosition.map((question) => {
    const leftShift = question.letterPosition
      ? question.letterPosition - 1
      : question.letterPosition;
...
export const placeQuestions = () => {
  const questionsWithLetterPosition: QuestionWithLetterPosition[] =
  ...
  const { maxLeftShift, maxRightShift } = calculateMaxShifts(
    questionsWithLetterPosition,
  );

  return calculateShifts(
    questionsWithLetterPosition,
    maxLeftShift,
    maxRightShift,
  );
